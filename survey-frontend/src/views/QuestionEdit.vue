...
export default {
  ...
  methods: {
    ...
    async stopSurvey() {
      const surveyId = this.$route.params.id;
      try {
        const response = await axios.patch(`https://seulgi97.duckdns.org/polls/surveys/${surveyId}/complete/`, { completed: true });
    ...
    async restartSurvey() {
      const surveyId = this.$route.params.id;
      try {
        const response = await axios.patch(`https://seulgi97.duckdns.org/polls/surveys/${surveyId}/start/`, { completed: false });
        if (response && response.data) {
    ...
    async deleteSurvey() {
      const surveyId = this.$route.params.id;
      try {
        const response = await axios.patch(`https://seulgi97.duckdns.org/polls/surveys/${surveyId}/delete/`, { deleted: true });
        if (response && response.data) {
